# STM — 주차 감지 노드 펌웨어 (STM1 / STM2 공용)

VEDA 4기 5팀 주차장 감시/제어 시스템의 센서 보드 펌웨어. NUCLEO-F401RE 기반으로
홀센서(GPIO 직결)와 불꽃센서(ADC+FFT)를 담당하고, 결과를 LoRa로 Raspberry Pi에
전달한다.

## ★ 어느 보드로 빌드할지 고르기

**`Core/Inc/board_config.h` 의 한 줄만 바꾸고 다시 빌드한다.**
...
|---|---|---|
| 노드 ID | `STM1` | `STM2` |
| 홀센서 | HALL01, HALL02 | HALL03, HALL04 |
| 불꽃센서 | 있음 | 없음 |
| LoRa 주소 | `0x0001` | `0x0002` |
| 빌드 크기 | 약 160 KB | 약 69 KB |

노드 ID·센서 번호·LoRa 주소가 **전부 이 한 줄에서 파생된다.** 따로 맞출 것이
없으므로 "노드 이름은 STM2 인데 주소는 STM1" 같은 조합이 나올 수 없다.

굽고 나면 **부팅 배너 첫 줄에서 확인한다.**
...
> 두 보드를 연달아 구울 때 이 줄을 안 보면, 같은 주소로 송신하는 노드
> 두 개를 만들어 놓고 링크가 왜 이상한지 한참 찾게 된다.

> **프로젝트를 STM1/ STM2/ 로 나누지 않는다.** 예전에 그렇게 했다가 전체
> diff 를 떠 보니 실제로 다른 것은 4줄이었고, 그 4줄 때문에 428개 파일이 두
> 벌 있었다. 문제는 용량이 아니라 한쪽만 고쳐도 빌드가 통과한다는 것이다.
