...
### 6-4. AUX 미연결 — 하행을 연달아 보내면 안 됩니다 ★

**서버 Pi의 E22 AUX 핀은 지금 연결돼 있지 않습니다.** 수신만 할 때는 문제가
없었지만, 하행 명령을 보내기 시작하면 얘기가 달라집니다.

AUX는 모듈이 바쁘면 LOW, 준비되면 HIGH가 되는 출력입니다. 이게 없으면
**"모듈이 이전 패킷을 다 보냈는지"를 알 방법이 없습니다.**

#### 무엇이 깨지나

모듈은 UART 스트림이 잠깐 멈추는 것을 패킷 경계로 씁니다. 그래서 프레임 두
개를 **간격 없이 연달아 write 하면 모듈이 둘을 한 패킷으로 붙여버립니다.**

노드가 둘이 되면서 이게 실제 위험이 됐습니다. HALL01(STM1)과 HALL03(STM2)에
LED 명령을 연달아 보낸다고 하면:

```
의도    [00 01 1E][프레임A]   [00 02 1E][프레임B]
실제    [00 01 1E][프레임A  00 02 1E  프레임B]
         └ STM1로            └ 목적지가 아니라 그냥 페이로드가 됨
```

**STM1은 쓰레기가 섞인 프레임을 받고(CRC 실패로 버림), STM2는 아무것도 못
받습니다.** 두 번째 명령이 조용히 사라지는 형태라 원인 찾기가 까다롭습니다.

STM 쪽에서 정확히 이 문제를 겪었습니다. 고정점 헤더를 본문과 나눠서 보냈더니
모듈이 앞 3바이트를 완결 패킷으로 끊고, 이어진 `AA 55`를 다음 패킷의 주소로
해석했습니다. 방향만 반대일 뿐 같은 원인입니다.

#### 대응 — 코드로 (권장, 배선 불필요)

**하행을 직렬화하고 사이에 간격을 두면 됩니다.** 두 가지를 지키십시오.

1. **한 프레임은 목적지 3바이트까지 포함해 한 번의 `write()`로** 내보낼 것
   (버퍼 하나로 합쳐서 쓰기. 나눠 쓰면 그 사이에서 잘립니다)
2. **프레임과 프레임 사이에 최소 50 ms**를 둘 것

50 ms의 근거는 STM 펌웨어가 AUX 없을 때 쓰는 폴백과 같은 계산입니다.

```
간격 = 공중점유(ms) + 40
     = (프레임길이+3) x 8 x 1000 / 62500 + 40

40바이트 프레임 -> 5.5 + 40 = 45.5 ms  ->  넉넉히 50 ms
```

`40`은 모듈 내부 처리와 LBT 재시도를 위한 여유입니다. **LBT가 켜져 있어서
채널이 바쁘면 송신이 더 밀리는데, AUX가 없으면 그 사실을 알 수 없습니다.**
노드가 둘이 되어 채널이 더 붐비므로 여유를 깎지 마십시오.

명령을 큐에 넣고 워커 하나가 50 ms 간격으로 꺼내 보내는 형태가 가장
안전합니다. 하행은 LED 제어 정도라 처리량이 문제 되지 않습니다.

#### 대응 — 배선으로 (더 확실함)

AUX를 물리면 추정 대신 **실제로 끝났는지**를 보고 보낼 수 있습니다.

| E22 패드 | Pi 핀 | BCM | 물리핀 |
|---|---|---|---|
| AUX | GPIO25 | 25 | 22 |

M0(16번)·M1(18번) 바로 옆이라 배선이 깔끔합니다. 읽는 법은 단순합니다.

```
HIGH = idle (보내도 됨)  /  LOW = busy (기다릴 것)
```

송신 전에 HIGH를 확인하고, 송신 후 HIGH로 돌아올 때까지 기다렸다가 다음
프레임을 보내면 됩니다. 타임아웃은 2초면 충분합니다(STM 펌웨어와 동일).

**입력에 풀업을 걸어두는 것을 권합니다.** 그러면 선이 빠져도 항상 HIGH로
읽혀 위의 "고정 간격" 동작으로 조용히 되돌아갑니다. 배선이 틀려도 퇴행이
없습니다.

> 급하지 않다면 **코드 쪽만 해도 충분합니다.** 지금 하행 트래픽이 적어서
> 50 ms 간격으로 잃는 게 없습니다. 다만 나중에 하행이 늘어나면 그때는
> AUX를 물리는 편이 낫습니다 — 고정 간격은 항상 최악을 가정해 기다리지만
> AUX는 실제로 끝나는 즉시 다음으로 넘어갑니다.
