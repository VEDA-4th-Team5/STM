...
AUX는 **수신만 한다면** 없어도 됩니다.

> **하행(Pi → STM) 명령을 보낼 거라면 읽어주세요.** AUX가 없으면 모듈이
> 이전 패킷을 다 보냈는지 알 수 없어서, 프레임 두 개를 연달아 write 하면
> 모듈이 둘을 한 패킷으로 붙여버립니다. 노드가 둘이라 목적지가 번갈아
> 나가는데, 그러면 **두 번째 명령이 조용히 사라집니다.**
>
> 배선 없이 넘어가려면 **프레임 사이에 최소 50 ms**를 두고 한 프레임은
> 목적지 3바이트까지 포함해 한 번의 `write()`로 내보내면 됩니다.
> 자세한 근거와 배선 방법은 `docs/PI_SERVER_LORA_V11_PATCH.md` §6-4 에
> 정리해 뒀습니다. 물릴 거면 GPIO25(물리 22번)가 M0/M1 옆이라 깔끔합니다.
